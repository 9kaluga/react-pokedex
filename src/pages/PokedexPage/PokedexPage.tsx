...
const POKEMON_COUNT_LIST = 50;

export const PokedexPage: React.FC = () => {
  const [offset, setOffset] = useState(POKEMON_COUNT_LIST);
        ...
        <div className={styles.card}>
          ...
          <div className={styles.card_types}>
            {selectedPokemon.types.map((type: TypeList) => (
              <div className={styles.card_type} key={type.type.name}>
                {type.type.name}
              </div>
            ))}
          </div>
          <div>
            <img src={selectedPokemon.sprites.front_default ?? ''} alt='pokemon img' />
          </div>
          <div className={styles.card_info}>
            <div className={styles.card_info_wrap}>
              <div className={styles.card_info_title}>Stats</div>
              <ul>
                {selectedPokemon.stats.map((stat: StatList) => (
                  <li className={styles.card_info_stat} key={stat.stat.name}>
            ...
            <div className={styles.card_info_wrap}>
              ...
              <ul>
                {selectedPokemon.abilities.map((ability: AbilityList) => (
                  <li className={styles.card_info_stat} key={ability.ability.name}>
        ...
        <div className={styles.list_wraper}>
          <ul className={styles.list}>
            {pokemons.map((pokemon) => (
              ...
              >
                <img
                  className={styles.pokemon_item_img}
                  src={pokemon.sprites.front_default ?? ''}
                  alt='pokemon img'
                />
                <div className={styles.pokemon_item_text}>{pokemon.name}</div>
              </li>
            ))}
          </ul>
        </div>
